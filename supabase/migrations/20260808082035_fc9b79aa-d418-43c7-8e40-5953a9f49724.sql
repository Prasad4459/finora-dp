ALTER TYPE public.asset_type ADD VALUE IF NOT EXISTS 'digital_gold';
ALTER TYPE public.asset_type ADD VALUE IF NOT EXISTS 'gold_etf';
ALTER TYPE public.asset_type ADD VALUE IF NOT EXISTS 'gold_fund';
ALTER TYPE public.asset_type ADD VALUE IF NOT EXISTS 'sovereign_gold_bond';
...
BEGIN
  ...
  ELSIF _t.type::text IN ('expense', 'investment', 'emi') THEN
    -- wallet_id may be NULL for employer-funded investment contributions
    -- (EPF / NPS): the asset grows but no bank balance moves.
    IF _t.wallet_id IS NOT NULL THEN
      UPDATE public.wallets SET balance = balance - amt WHERE id = _t.wallet_id;
    END IF;
  ...
  IF _t.type::text = 'investment' AND _t.asset_id IS NOT NULL THEN
    UPDATE public.assets
      SET current_value = current_value + amt,
          ...
          is_active = CASE WHEN _sign > 0 THEN true ELSE is_active END
  ...
  IF _t.type::text = 'redemption' AND _t.asset_id IS NOT NULL THEN
    ...
    WHERE id = _t.asset_id
    RETURNING units, current_value INTO a_units, a_value;

    -- A fully redeemed holding is kept for history but flagged inactive.
    -- Reversing the redemption (edit/delete) brings it back.
    UPDATE public.assets
      SET is_active = NOT (
            COALESCE(a_value, 0) <= 0
            AND (a_units IS NULL OR COALESCE(a_units, 0) <= 0)
          )
    WHERE id = _t.asset_id;
  END IF;
