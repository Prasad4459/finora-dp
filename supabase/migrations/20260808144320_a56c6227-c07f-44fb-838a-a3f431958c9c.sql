CREATE UNIQUE INDEX IF NOT EXISTS bill_payments_bill_occurrence_uidx
  ON public.bill_payments (bill_id, period_key);
...
BEGIN
  ...
  v_period_key := v_bill.due_date::text;
  ...
  IF v_bill.is_recurring THEN
    IF _next_due_date IS NULL OR _next_due_date <= v_bill.due_date THEN
...
REVOKE ALL ON FUNCTION public.pay_bill_occurrence(uuid, numeric, uuid, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.pay_bill_occurrence(uuid, numeric, uuid, date, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pay_bill_occurrence(uuid, numeric, uuid, date, date) TO service_role;