...
export function Liabilities() {
  ...
  const isLoading = liabilitiesData.isLoading;
  const isError = liabilitiesData.isError;
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard
          ...
          tone="negative"
          state={state}
          onRetry={liabilitiesData.refetch}
        />
        <StatCard
          label="Monthly EMI"
          value={formatINR(emi)}
        ...
        <StatCard
          ...
          value={String(liabilities.length)}
          icon={CreditCard}
          state={state}
          onRetry={liabilitiesData.refetch}
        />
      ...
      <Card className="mt-6 border-border/70">
        <CardHeader>
          <CardTitle className="text-base font-semibold">All liabilities</CardTitle>
        </CardHeader>
        <CardContent className={cn(liabilities.length > 0 && state === "ready" && "p-0")}>
          {isError ? (
            <WidgetError message="Couldn't load your liabilities." onRetry={liabilitiesData.refetch} />
          ) : isLoading ? (
            <WidgetSkeleton lines={5} />
          ) : liabilities.length === 0 ? (
            <WidgetEmpty
              message="Nothing owed yet. Add a loan or credit card to track outstanding balances and EMIs."
              actionLabel="Add your first liability"
              onAction={() => openDialog("liability")}
            />
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {liabilities.map((l) => (
                  <TableRow key={l.id}>
                    ...
                    <TableCell className="whitespace-nowrap text-right tabular-nums">{l.rate}%</TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant={l.status === "Due" ? "destructive" : "secondary"}
                        className="text-[10px]"
                      >
                        {l.status}
                      </Badge>
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        <Button
                          ...
                          onClick={() => openEditDialog({ kind: "liability", entity: l })}
                        >
